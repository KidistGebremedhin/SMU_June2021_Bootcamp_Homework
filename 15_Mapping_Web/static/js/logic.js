...
function makeMap(data, tect_data) {
    ...
    for (var i = 0; i < features.length; i++) {
        ...
        if (location) {
            ...
            marker.bindPopup(`<h3>${features[i].properties.place} Mag: ${features[i].properties.mag}</h3><hr><p>${new Date(features[i].properties.time)}</p>`);
            ...
            }).bindPopup(`<h3>${features[i].properties.place} Mag: ${features[i].properties.mag}</h3><hr><p>${new Date(features[i].properties.time)}</p>`);
